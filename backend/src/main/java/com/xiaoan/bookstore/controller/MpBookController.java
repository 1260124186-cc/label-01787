...
public class MpBookController {
    ...
    @Log("上传书籍")
    public Result<Book> upload(HttpServletRequest request,
    ...
    @GetMapping("/books/{id}")
    public Result<Book> detail(HttpServletRequest request, @PathVariable Long id) {
        Long userId = (Long) request.getAttribute(Constants.CONTEXT_USER_ID);
    ...
    public Result<List<Map<String, Object>>> getToc(HttpServletRequest request, @PathVariable Long id) {
        Long userId = (Long) request.getAttribute(Constants.CONTEXT_USER_ID);
        return Result.success(bookService.getToc(userId, id));
    }

    @GetMapping("/books/{id}/text/{pageNum}")
    public Result<String> getPageText(HttpServletRequest request,
                                       @PathVariable Long id,
                                       @PathVariable int pageNum) {
        Long userId = (Long) request.getAttribute(Constants.CONTEXT_USER_ID);
    ...
    public Result<Void> updateProgress(HttpServletRequest request,
                                        @PathVariable Long id,
    ...
    @Log("创建分类")
    public Result<Category> createCategory(HttpServletRequest request, @Valid @RequestBody CategoryDTO dto) {
        Long userId = (Long) request.getAttribute(Constants.CONTEXT_USER_ID);
        return Result.success(categoryService.create(userId, dto));
    }

    @GetMapping("/categories")
    public Result<List<Category>> categoryList(HttpServletRequest request) {
        Long userId = (Long) request.getAttribute(Constants.CONTEXT_USER_ID);
    ...
    @Log("更新分类")
    public Result<Void> updateCategory(HttpServletRequest request, @PathVariable Long id,
